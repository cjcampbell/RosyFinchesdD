...
feather_iso <- raster::raster(file.path(wd$bin, "featherIsoscape.tif"))
feather_se  <- raster::raster(file.path(wd$bin, "julsep_se.tif"))
    ...
    if(nrow(df) > 0) {
      pbmcapply::pbmclapply(1:nrow(df), mc.cores = 3, function(i) {
        ...
        if(mySampleType == "Feather") {
          myIsoscape <- feather_iso
          myIsoscape_sd <- feather_se
